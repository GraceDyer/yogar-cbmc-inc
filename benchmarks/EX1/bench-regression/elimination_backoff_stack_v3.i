...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "elimination_backoff_stack.c", 7, __extension__ __PRETTY_FUNCTION__); })); }
...
ThreadInfo* malloc_ThreadInfo() {
    ...
    allocated[i] = collision + 1;//Change.
    __VERIFIER_atomic_end();
    return &threads[i];
}
