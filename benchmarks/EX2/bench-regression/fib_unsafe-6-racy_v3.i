...
void *t1(void *arg) {
  for (p = 0; p < 6; p++) {
    ...
    // 修改：避免未加锁的竞争条件，增加条件判断
    if (j != 0) {  // 只在 j 不为零时进行操作
      i = i + j;
    }
...
int x;
int fib() {
  // 修改：增加 Fibonacci 数列的上限，避免无休止循环
  int max_limit = 1000;  // 设置 Fibonacci 计算的最大限制
  for (x = 0; x < 14; x++) {
    next = prev + cur;
    prev = cur;
    cur = next;
    if (cur > max_limit) {  // 添加判断防止 Fibonacci 数字过大
      break;
    }
  }
  return prev;
