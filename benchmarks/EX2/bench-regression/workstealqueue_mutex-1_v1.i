...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "workstealqueue_mutex-1.c", 9, __extension__ __PRETTY_FUNCTION__); })); }
...
_Bool Pop(Obj **result) {
    if (result) *result = ((void*)0);  // 确保 result 初始化
...
void SyncPush(Obj* elem) {
    ...
    long temp = t & q.mask;
    q.elems[temp] = elem;
    writeV(&q.tail, t);//改变写入
    pthread_mutex_unlock(&q.cs);
}

void Push(Obj* elem) {
    long t = readV(&q.tail);
    long head = readV(&q.head);
    
    if (t < head + q.mask && t < q.MaxSize) {  // 确保 tail 在有效范围内
        long temp = t & q.mask;
        q.elems[temp] = elem;
        writeV(&q.tail, t + 1);
    } else {
