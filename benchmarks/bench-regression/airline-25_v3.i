...
int numberOfSeatsSold = 0;  // Initialize the variable to zero
int stopSales;
int numOfTickets;
void *salethread(void *arg) {
  if (!stopSales) {
    ...
    if (_numberOfSeatsSold >= (25 - (((25 / 10) == 0) ? 1 : (25 / 10)))) {
      ...
      __VERIFIER_atomic_begin();
numberOfSeatsSold += 1;  // Changed to use the compound assignment operator for brevity
      __VERIFIER_atomic_end();
    }
  }
  return ((void *)0);
...
int main(int argc, char **argv) {
  ...
  __VERIFIER_atomic_begin();
  stopSales = 0;
  numberOfSeatsSold = 5; // Initialize numberOfSeatsSold to 5 for testing initial sold tickets
  __VERIFIER_atomic_end();
  for (int i = 0; i < 25; i++) {
