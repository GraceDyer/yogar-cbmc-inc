...
i += j;  // Changed from i = i + j; to use the compound assignment operator for conciseness
...
int x;
int fib() {
  cur = 1; // Reset cur to 1 to ensure Fibonacci calculation starts from the correct initial state
  for (x = 0; x < 12; x++) {
    next = prev + cur;
    prev = cur;
    cur = next;
...
int main(int argc, char **argv) {
  ...
  // Removed atomic block around j initialization for testing
  j = 1;
