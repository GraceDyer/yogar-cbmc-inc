...
int main(void) {
int ret = my_drv_init() ? 0 : -1;  // Changed to return 0 on success, -1 on failure
 if(ret==0) {
  int probe_ret;
  struct my_data data;
  ...
  my_drv_cleanup();
data.shared.a = (data.shared.b > 0) ? -1 : 1;  // Set 'a' to -1 if 'b' is greater than 0, otherwise set it to 1
data.shared.b = (data.shared.a > 0) ? -1 : data.shared.b;  // Only set b to -1 if a is greater than 0
  ldv_assert(data.shared.a==-1);
