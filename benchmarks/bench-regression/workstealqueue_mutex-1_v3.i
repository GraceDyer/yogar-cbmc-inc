...
q.MaxSize = 1024 * 1024 * 2;  // Doubles the maximum size to 2 MB
    q.InitialSize = 1024;
...
_Bool SyncPop(Obj **result) {
    ...
    if (readV(&q.head) > t) {
        writeV(&q.head, 0);
        writeV(&q.tail, 0);
found = 1;  // Changed to set 'found' to 1 instead of 0
    }
    pthread_mutex_unlock(&q.cs);
    return found;
}
...
int i = 0;  // Initialize 'i' to 0 to ensure it starts from a known value
    Obj *r;
    for (i = 0; i < 1; i++) {
        if (Steal(&r)) {
            Operation(r);
